...
;(async()=>{
  ...
  for(const dia of DIAS){
    ...
    execSync(`npx tsx scripts/analise/gerar_kpi_from_files.ts ${dia}`,{stdio:"ignore"})
    console.log(`\n══════ DIA ${dia} ══════`)
    for(const [rede,fm] of PARES){
      ...
      try{ wbB=new ExcelJS.Workbook(); await wbB.xlsx.readFile(`${BETA_DIR}/KPI-${rede}-2026-05-${dia}-FF.xlsx`) }catch{continue}
