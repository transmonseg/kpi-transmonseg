/**
 * Análise placa a placa — ZONA SUL 19/05/2026
...
async function main() {
  ...
  const escalaRaw = await parseEscalaZonaSul(readFileSync(BASE + '/ESCALA ZONA SUL - MAIO (6).xlsx'), DATA)
  ...
  const veiculos = await parseUnitrac(readFileSync(BASE + '/relatorio_9391.xlsx'))
  ...
  const [kpiM, kpiG] = await Promise.all([
    lerKpi('C:/Users/media/Downloads/KPI-ZONA_SUL-2026-05-19.xlsx'),
    lerKpi('C:/Users/media/Downloads/KPI-ZONA_SUL-2026-05-19 (4).xlsx'),
  ])
  ...
  process.stdout.write('=== ANÁLISE PLACA A PLACA — ZONA SUL 19/05/2026 ===\n')
  process.stdout.write('SC=SaídaCD  CHD=ChegadaLoja  SL=SaídaLoja\n\n')

  // --- e SEM são equivalentes (sem dados = sem rastreador)
  function noData(v: string): boolean { return v === '---' || v.startsWith('SEM') }
  function arrEq(a: string[], b: string[]): boolean {
    return a.every((v, i) => (noData(v) && noData(b[i])) || v === b[i])
  }

  // Resolve qual slot manual (1 ou 2) corresponde à placa, comparando sem espaços/hífens
  ...
  for (const [loja, slots] of lojaMap) {
    ...
    for (const l of slots) {
      ...
      // Determina slot manual pela placa (carro_ordem sempre=1 no parser Zona Sul)
      const ks = resolveSlot(l.placa_norm, km)
      const ksG = resolveSlot(l.placa_norm, kg)
      const mArr = ks === 1
      ...
      const slotLabel = ks === 0 ? `c${slot}?` : `c${slot}→s${ks}`
