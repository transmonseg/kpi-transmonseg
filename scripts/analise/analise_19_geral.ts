/**
 * Análise dia 19/05/2026 — redes da Escala Geral + PAX + Armazém + Guanabara
 * Uso: npx tsx scripts/analise/analise_19_geral.ts <REDE_ID>
 * Ex: npx tsx scripts/analise/analise_19_geral.ts SUPER_PAX
 */
...
// Qual rede analisar (argumento CLI)
const REDE_ID = process.argv[2]?.toUpperCase()
if (!REDE_ID) {
  console.error('Uso: npx tsx scripts/analise/analise_19_geral.ts <REDE_ID>')
  console.error('Redes disponíveis: SUPER_PAX FEIRA_NOVA MUNDIAL SENDAS CARREFOUR ATACADAO ASSAI PREZUNIC VIANENSE PRINCESA SUPERPRIX SUPERCOMPRAS SAMS_CLUB CAB_PETROPOLIS ARMAZEM_GRAO GUANABARA')
  process.exit(1)
}
...
async function main() {
  ...
  const [kpiM, kpiG] = await Promise.all([
    lerKpi(DL + `KPI-${REDE_ID}-2026-05-19.xlsx`),
