// Mede QUALIDADE do match (não quantidade): pra cada match, verifica se o
// nome da parada bate semanticamente com o nome da loja da escala.
// Output: count de matches certos vs matches duvidosos vs matches errados.
...
function tokensNome(s: string): Set<string> {
  return new Set(s
    ...
    .replace(/SENDAS|ASSA[IÍ]|PREZUNIC|CARREFOUR|SUPERPRIX|ATACAD[AÃ]O|VIANENSE|SAM'?S|EMANUEL|FEIRA|NOVA|PRINCESA|ARMAZEM|GR[AÃ]O|REGINA|MUNDIAL|GUANABARA|ZONA|SUL|LOJA|LJ\.?|REDE|MERCADO|SUPERMERCADO/g, ' ')
    .split(/[^A-Z0-9]+/)
    .filter(t => t.length >= 3))
}
...
function nomeMatches(escalaNome: string, paradaNome: string | null): 'OK' | 'OK_PARCIAL' | 'DUVIDOSO' | 'ERRADO' {
  ...
  const te = tokensNome(escalaNome)
  const tp = tokensNome(paradaNome)
...
async function main() {
  const ger = await parseEscalaGeral((await readFile(`${ROOT}/ESCALA GERAL DE MAIO 1 (6).xlsx`)).buffer.slice(0, undefined) as ArrayBuffer, DATA)
  const zs = await parseEscalaZonaSul((await readFile(`${ROOT}/ESCALA ZONA SUL - MAIO (6).xlsx`)).buffer.slice(0, undefined) as ArrayBuffer, DATA)
  const arm = await parseEscalaArmazemGrao((await readFile(`${ROOT}/ESCALA DO ARMAZÉM DO GRÃO MAIO (5).xlsx`)).buffer.slice(0, undefined) as ArrayBuffer, DATA)
  const gua = await parseEscalaGuanabaraPdf(await readFile(`${ROOT}/ESCALA 19.05.pdf`), DATA)

  let escalaLinhas: LinhaEscala[] = [...ger, ...zs, ...arm, ...gua]
  ...
  const counts: Record<string, { OK: number; OK_PARCIAL: number; DUVIDOSO: number; ERRADO: number; UNMATCHED: number; total: number }> = {}
  ...
  for (const rota of rotas) {
    ...
    if (!counts[rede]) counts[rede] = { OK: 0, OK_PARCIAL: 0, DUVIDOSO: 0, ERRADO: 0, UNMATCHED: 0, total: 0 }
    counts[rede].total++
    if (rota.paradas.length === 0) { counts[rede].UNMATCHED++; continue }
    const paradaNome = rota.paradas[0].nome
    const alg = rota._matchMeta?.algorithm ?? ''
    // Geo, trgm e Guanabara-via-codigo: nome pode ser genérico mas match é
    // semanticamente certo (resolveLojaId casou pelo código ou coords)
    if (alg === 'geo' || alg === 'trgm' || rota.paradas[0].loja_id) {
      counts[rede].OK++
      continue
    }
    const q = nomeMatches(esc.loja_nome_raw, paradaNome)
    counts[rede][q]++
    if (q === 'ERRADO') erradosList.push({ rede, escala: esc.loja_nome_raw, parada: paradaNome })
  }

  console.log('\nRede                | Total | OK    | Parcial | Duvid | Errado | Unmatched | Qualidade %')
  console.log('--------------------|-------|-------|---------|-------|--------|-----------|------------')
  let totG = 0, okG = 0
  for (const [r, c] of Object.entries(counts).sort((a, b) => b[1].total - a[1].total)) {
    const corretos = c.OK + c.OK_PARCIAL
    const pct = c.total > 0 ? Math.round((corretos / c.total) * 100) : 0
    console.log(`${r.padEnd(19)} | ${String(c.total).padStart(5)} | ${String(c.OK).padStart(5)} | ${String(c.OK_PARCIAL).padStart(7)} | ${String(c.DUVIDOSO).padStart(5)} | ${String(c.ERRADO).padStart(6)} | ${String(c.UNMATCHED).padStart(9)} | ${String(pct).padStart(3)}%`)
    totG += c.total
    okG += corretos
  }
  console.log('--------------------|-------|-------|---------|-------|--------|-----------|------------')
  const pctG = totG > 0 ? Math.round((okG / totG) * 100) : 0
  console.log(`TOTAL               | ${String(totG).padStart(5)} | ${String(okG).padStart(5)} (qualidade ${pctG}%)`)

  if (erradosList.length > 0) {
    console.log(`\n${erradosList.length} matches ERRADOS (nome da parada não tem nenhum token em comum):`)
    for (const e of erradosList.slice(0, 20)) console.log(`  [${e.rede}] "${e.escala}" → "${e.parada}"`)
  }
}
