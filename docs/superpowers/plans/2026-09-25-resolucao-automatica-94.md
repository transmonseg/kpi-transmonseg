...
### Task 2b: Trava de regressão com dias reais rotulados (obrigatória antes de todo deploy)

**Files:** Create `scripts/verificar-kpi-gabaritos.py` e `scripts/gabaritos/casos-rotulados.csv`.

- [ ] `casos-rotulados.csv` (colunas `data;placa;nf;esperado;fonte`), com `esperado ∈ {entregue, nao_entregue, sem_rastreador, desatualizado}`, gerado das abas 22-09/23-09/24-09 de `~/ClaudeGerado/kpi-regen-0922/grupo/3EB01CE5F465CF35B40C16_Ocorrencias KPI 24-09.xlsx` (RESOLUÇÃO "Entregue…" → entregue; "Não foi/esteve…" → nao_entregue; "Desatualizado" → desatualizado) + casos fixos: 2026-09-23 TTL5J17 (todas as NFs) → sem_rastreador; 2026-09-23 RQQ5B81 2386225 → nao_entregue; 2026-09-24 RQU3F71 2388098 e RQV9B26 2388278 → nao_entregue.
- [ ] `verificar-kpi-gabaritos.py <xlsx_22> <xlsx_23> <xlsx_24>`: para cada caso, lê status/observação da NF no xlsx do dia e classifica: `nao_entregue` que saiu com status começando por `ENTREGUE` = **FALHA CRÍTICA** (falso positivo); `sem_rastreador` que não saiu com o rótulo SEM RASTREADOR = FALHA; `entregue` que saiu `ENTREGUE` = acerto (reportar taxa de acerto, sem falhar). Também roda `scripts/medir-contra-gabarito-ana.py` no 22/09 e falha se ≤2 min < 988, ≤10 min < 1104 ou sem horário > 29. Sai com código 1 se houver qualquer falha; imprime a lista de NFs que falharam.
- [ ] Baseline: rodar contra os xlsx de produção atuais (código ad01b19) e registrar no relatório quais casos falham HOJE (falsos positivos atuais conhecidos, ex.: RBG2D21 2389318, TTH3C94 2388733, RBG5G18 2388403, RQS7H76 2389481) — esses entram como `conhecido` numa lista de exceções com data, para a trava não bloquear por erro antigo, mas bloquear qualquer caso NOVO que quebre.
- [ ] Task 3 só faz deploy se `verificar-kpi-gabaritos.py` sair 0 com o código novo.
