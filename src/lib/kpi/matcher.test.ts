...
describe('T18 — plate-swap aceita madrugada + GPS:NAO', () => {
  it('T18 ACEITA parada às 05:00 BRT (madrugada legítima ZS cross-day)', async () => {
    ...
    const paradas: UnitracParadaRow[] = [
      { id: 'pbase', placa_norm: 'ABC1234', chegada: '2026-05-20T03:00:00.000Z', saida: '2026-05-20T04:30:00.000Z', duracao_seg: 5400, local_parada: 'BASE', codigo_loja: null, nome_loja: null, lat: null, lng: null, classificacao: 'BASE', ordem: 0 },
  ...
  it('T18 ACEITA parada de outra placa às 07:00 BRT quando veículo TEM GPS (T18-G passa)', async () => {
    // ABC tem GPS (BASE parada). XYZ tem LOJA às 07:00 BRT (T18-N passa).
    // T18-G: ABC está no Unitrac → T18 roda → match encontrado.
    const linhas: EscalaLinhaRow[] = [
    ...
    const paradas: UnitracParadaRow[] = [
      { id: 'pbase2', placa_norm: 'ABC1234', chegada: '2026-05-20T03:00:00.000Z', saida: '2026-05-20T06:00:00.000Z', duracao_seg: 10800, local_parada: 'BASE', codigo_loja: null, nome_loja: null, lat: null, lng: null, classificacao: 'BASE', ordem: 0 },
  ...
  it('T18 RODA para veículo GPS:NAO (placa ausente — caso plate-swap real)', async () => {
    ...
    const paradas: UnitracParadaRow[] = [
      // Apenas XYZ presente — ABC está completamente fora do Unitrac
...
describe('Bug 3 — parada LOJA duplicada não deve bloquear geo-fallback FORA_BASE', () => {
  it('2 paradas LOJA cod=9039030 (duplicata) + Loja 47 noite via FORA_BASE geo → Loja 47 casa via geo', async () => {
    ...
    // T22 (2026-05-27 v2): FORA_BASE com GPS exatamente no raio do cadastro
    // vira LOJA. Manual da Tia Erica preenche entrega nesses casos (Atacadão
    // Manilha QSS-1E48 dia 19 confirmou). Loja 47 ganha a parada 19:40.
...
describe('Bug 4 — multi-row mesma rede: distribuição correta entre N linhas e M paradas', () => {
  ...
  it('Bug 6: ATACADAO MANILHA dia 19 (matched=FORA_BASE → cadeia FORA_BASE longa, mesma área)', async () => {
    ...
    // T22: GPS no raio do cadastro = entrega real (manual da Tia confirma).
