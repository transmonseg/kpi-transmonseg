  ...
  return (
    <div className="mx-auto w-full max-w-[1200px]">
      ...
      <header className="mb-10 flex flex-col gap-1.5">
        ...
        <h1 className="text-[28px] font-semibold capitalize leading-tight tracking-[-0.02em] text-[var(--color-fg)] md:text-[34px]">
          {formatHoje(status.hojeIso)}
        </h1>
      </header>

      {/* Bento asymmetric: hero 8/12 + lateral 4/12 (Section 9 do design-taste-frontend) */}
      <section className="grid grid-cols-1 gap-4 lg:grid-cols-12">
        {/* HERO — KPI do dia */}
        <Link
          href={`/painel/kpi/dia?data=${status.hojeIso}`}
          className="group relative col-span-1 flex flex-col justify-between overflow-hidden rounded-[var(--radius-card)] border border-[var(--color-border)] bg-[var(--color-bg-elevated)] p-8 transition-all duration-200 hover:border-[var(--color-border-strong)] active:scale-[0.997] md:p-10 lg:col-span-8 lg:min-h-[320px]"
        >
          {/* Tinted ambient glow ao fundo (skill: radial-gradient sutil, opacity 0.04) */}
          <div
            aria-hidden
            className="pointer-events-none absolute -top-32 -right-32 h-[400px] w-[400px] rounded-full opacity-[0.06]"
          ...
          <div className="relative flex items-start justify-between gap-6">
            ...
            <ArrowUpRight
              size={20}
              weight="bold"
              className="text-[var(--color-fg-subtle)] transition-transform duration-200 group-hover:-translate-y-0.5 group-hover:translate-x-0.5 group-hover:text-[var(--color-fg)]"
            />
          </div>

          <div className="relative mt-10 flex flex-col gap-3">
            <span className="text-display text-[68px] text-[var(--color-fg)] md:text-[88px]">
              {kpi.headline}
            </span>
        ...
        <div className="col-span-1 grid grid-cols-1 gap-4 lg:col-span-4">
          ...
          <Link
            href="/painel/revisao"
            className="group relative flex flex-col overflow-hidden rounded-[var(--radius-card)] border border-[var(--color-border)] bg-[var(--color-bg-elevated)] p-6 transition-all duration-200 hover:border-[var(--color-border-strong)] active:scale-[0.997]"
          >
            ...
            <div className="mt-4 flex items-baseline gap-2.5">
              <span
                className={`text-display text-[48px] ${
          ...
          <Link
            ...
            className="group flex flex-col rounded-[var(--radius-card)] border border-[var(--color-border)] bg-[var(--color-bg-elevated)] p-6 transition-all duration-200 hover:border-[var(--color-border-strong)] active:scale-[0.997]"
          >
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-2 text-[11px] font-medium uppercase tracking-[0.18em] text-[var(--color-fg-subtle)]">
      ...
      {/* Linhas operacionais — anti-card via divide-y (skill Rule 4: Anti-Card Overuse) */}
      <section className="mt-14">
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-[11px] font-medium uppercase tracking-[0.18em] text-[var(--color-fg-subtle)]">
            Status operacional
          </h2>
      ...
      {/* Acessos secundários — 2 cards lado a lado */}
...
function StatusRow({
  ...
  return (
    <div className="grid grid-cols-[1fr_auto_auto] items-center gap-6 py-4">
...
function SecondaryLink({
  ...
  return (
    <Link
      href={href}
      className="group flex flex-col gap-2 rounded-[var(--radius-card)] border border-[var(--color-border)] bg-[var(--color-bg-elevated)] p-6 transition-all duration-200 hover:border-[var(--color-border-strong)] active:scale-[0.997]"
    >
      <div className="flex items-center justify-between">
        <span className="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-[var(--color-border)] bg-[var(--color-bg)] text-[var(--color-fg-muted)] transition-colors group-hover:text-[var(--color-fg)]">
          {icon}
        </span>
        <ArrowUpRight
          size={16}
      ...
      <h3 className="mt-2 text-[15px] font-medium tracking-tight text-[var(--color-fg)]">
        {title}
      </h3>
