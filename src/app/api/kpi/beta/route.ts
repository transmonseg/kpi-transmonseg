...
export async function POST(req: NextRequest) {
  ...
  // KPI BETA: versão de teste — NUNCA persiste no banco de produção.
  // (bloco de persistência da rota /simples removido de propósito nesta beta)
  ...
  try {
    ...
    for (const loja of lojasParaMatcher) {
      ...
      if (loja.lat == null || loja.lng == null) {
        sugestoesCadastro.push({ codigo: p.cod, nome: p.nome, tipo: 'sem_coord', detalhe: `API tem: ${p.lat.toFixed(5)}, ${p.lon.toFixed(5)}` })
      } else {
        const d = haversine(loja.lat, loja.lng, p.lat, p.lon)
        if (d > 150) sugestoesCadastro.push({ codigo: p.cod, nome: p.nome, tipo: 'coord_errada', detalhe: `${Math.round(d)}m de erro (API: ${p.lat.toFixed(5)}, ${p.lon.toFixed(5)})` })
      }
    }

    // 2) Gatilho rota: marca previews cuja placa ainda está rodando ao vivo na API
    for (const rede of results as Array<{ preview?: PreviewLinha[] }>) {
      for (const pv of rede.preview ?? []) {
        if (!pv.placa) continue
        const v = validarRotaConcluida(pv.placa, posicoesApi)
        if (v?.aindaRodando) {
          pv.viaApi = [...(pv.viaApi ?? []), 'rota: ainda rodando']
          correcoesViaApi++
        }
      }
    }
